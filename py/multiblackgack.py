...
if allbot:
    player = list({"name":"Bot", "hand":Hand(), "isBot":True}for i in range(szplayer))
else:
    player = list({"name":input(f"player {i+1}'s name(type bot for Bot): "), "hand":Hand(), "isBot":False}for i in range(szplayer))
    for i in player:
    ...
    for p in player:
        if p["hand"].stillplay and not p["isBot"]:
            ...
            if sum(p["hand"].number) >= 17:
                ...
                p["hand"].draw()
        if not p["hand"].cadidate and not p["hand"].chkifexceed():
                alllose += 1
        elif p["hand"].cadidate:
            p["hand"].chkifexceed()
    turn += 1 
